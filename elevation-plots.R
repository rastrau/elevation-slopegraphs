...
df <- read_csv(here("elevations.csv"))
df <- df %>% 
...
# Third attempt: Plot an areal slope graph with sky and a custom theme
theme <- theme_void(base_size = 12) +
  theme(plot.tag = element_text(size = rel(1.5), lineheight = 1.15, 
                                hjust = 0, face = "bold"))+
...
r <- ggplot(df, aes(x = pos, y = max(df$elevation))) +
  ...
  theme +
  labs(tag = "elevations\naltitudine\nautezzas\naltitudes\nhöhen",
       caption = "@rastrau, idea by @philshem") +
  theme(plot.tag.position = c(0, 0.885))

ggsave(here("elevations.png"), r, width = 25, height = 19.3, units = "cm")
